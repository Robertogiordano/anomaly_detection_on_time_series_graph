"""
File: process_netcdf_to_csv.py

Description:
This script processes NetCDF files containing satellite observation data to extract and aggregate relevant information 
into a structured CSV format. It employs parallel processing for efficiency and handles masking of values based on 
quality control (QC) indicators.

Key Features:
- Reads NetCDF files using `xarray` and processes them to extract data based on a user-defined mapping.
- Supports masking of values based on QC flags to ensure data quality.
- Uses `ProcessPoolExecutor` for parallel file processing to improve performance with large datasets.
- Outputs the processed data into a consolidated CSV file.

Requirements:
- Python libraries: os, pandas, xarray, numpy, concurrent.futures, functools
"""
